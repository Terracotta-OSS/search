...
/**
 *  Copy of original lucene-core distribution version + backported patch for LUCENE-5931
 */
final class StandardDirectoryReader extends DirectoryReader {
  ...
  private static DirectoryReader open(Directory directory, SegmentInfos infos, List<? extends AtomicReader> oldReaders,
    ...
    for (int i = infos.size() - 1; i>=0; i--) {
      ...
      boolean success = false;
      readerShared[i] = false; // assume no sharing
      ...
      try {
        if (newReaders[i] == null || infos.info(i).info.getUseCompoundFile() != newReaders[i].getSegmentInfo().info.getUseCompoundFile()) {
          ...
          newReaders[i] = new SegmentReader(infos.info(i), termInfosIndexDivisor, IOContext.READ);
        } else {
          if (newReaders[i].isNRT) {
          ...
          if (newReaders[i].getSegmentInfo().getDelGen() == infos.info(i).getDelGen()
            ...
            if (newReaders[i].getSegmentInfo().getDelGen() == infos.info(i).getDelGen()) {
                // only DV updates
                newReaders[i] = new SegmentReader(infos.info(i), newReaders[i], newReaders[i].getLiveDocs(), newReaders[i].numDocs(), false);
              } else {
